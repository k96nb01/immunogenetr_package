...
Expression suffixes and G/P groups are handled on a simple rule: a
search without a suffix matches the allele regardless of expression
status or group membership, while a search carrying a suffix matches
only alleles with that same letter, at the same or higher resolution. A
G or P group search matches the group name only, not its member alleles:
...
The matched text is always the full allele as it appears in the GL
String, so
...
The `HLA-` prefix is optional in both the allele and the GL String —
wild-caught GL Strings often omit it — so an allele in either convention
matches a GL String in either convention:
...
Because an unsuffixed search matches expression variants, workflows that
should ignore non-expressed alleles can remove them first with
[`GLstring_drop_non_expressed()`](https://immunogenetr.org/reference/GLstring_drop_non_expressed.md).
The default removes N, S and C alleles, which produce no cell-surface
protein; the `suffixes` argument adjusts that judgement:
