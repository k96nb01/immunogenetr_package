...
HLA_mismatch_base <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH"), homozygous_count = 2) {
  ...
  non_null_alleles <- non_null_alleles[non_null_alleles != ""]
    ...
    if (length(non_null_alleles) == 1 && homozygous_count == 2) {
      alleles <- rep(non_null_alleles, times = homozygous_count)
    }
    
    # Return processed alleles as a single GL string
    return(str_c(alleles, collapse = "+"))
  }
  ...
  process_pair <- function(recip_str, donor_str) {
    ...
    mismatch_results <- map(loci, function(locus_name) {
      ...
      # Handle null alleles: if all alleles are null, return "NA"
      ...
      if (length(recip_valid) == 0 && length(donor_valid) == 0) {
        return(paste0(locus_name, "=NA"))
      }
      
      if (direction == "GvH") {
        # Mismatches present in the recipient but not in the donor
        mismatched_allele_1 <- setdiff(unlist(strsplit(recip_valid,"\\+"))[1], unlist(strsplit(donor_valid,"\\+")))
        mismatched_allele_2 <- setdiff(unlist(strsplit(recip_valid,"\\+"))[2], unlist(strsplit(donor_valid,"\\+")))
        ...
        mismatched_allele_1 <- setdiff(unlist(strsplit(donor_valid,"\\+"))[1], unlist(strsplit(recip_valid,"\\+")))
        mismatched_allele_2 <- setdiff(unlist(strsplit(donor_valid,"\\+"))[2], unlist(strsplit(recip_valid,"\\+")))
        mismatched_alleles <- discard(c(mismatched_allele_1, mismatched_allele_2), is.na)
      } else {
...
HLA_mismatch_number <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "bidirectional"), homozygous_count = 2) {
  ...
  # Helper function to format mismatch counts for loci
  format_output <- function(counts, loci) {
    if (length(loci) == 1) {
      return(counts)
    } else {
      return(paste(paste0(loci, "=", counts), collapse = ", "))
    }
  }
  
  # Calculate mismatch counts for both directions
  if (length(loci) == 1) {
    mismatch_count_HvG <- count_mismatches(mismatch_HvG)
    mismatch_count_GvH <- count_mismatches(mismatch_GvH)
  } else {
    mismatch_count_HvG <- sapply(strsplit(mismatch_HvG, ", ")[[1]], function(x) count_mismatches(strsplit(x, "=")[[1]][2]))
    mismatch_count_GvH <- sapply(strsplit(mismatch_GvH, ", ")[[1]], function(x) count_mismatches(strsplit(x, "=")[[1]][2]))
  }
  
  # Handle directions
  if (direction == "HvG") {
    return(as.integer(format_output(mismatch_count_HvG, loci)))
  } else if (direction == "GvH") {
    return(as.integer(format_output(mismatch_count_GvH, loci)))
  } else if (direction == "bidirectional") {
    max_counts <- pmax(mismatch_count_HvG, mismatch_count_GvH, na.rm = TRUE)
    return(as.integer(format_output(max_counts, loci)))
  }
...
# Single locus does not appear to be working (this should output "2", but instead outputs "HLA-A*03:01+HLA-A*74:01=NA")
HLA_mismatch_number(
  "HLA-A*03:01+HLA-A*74:01^HLA-DRB3*03:01^HLA-DRB5*02:21", 
  "HLA-A*03:02+HLA-A*20:01^HLA-DRB3*03:01",
  c("HLA-A"),
