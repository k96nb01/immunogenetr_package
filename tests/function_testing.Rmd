...
HLA_mismatch_base <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH"), homozygous_count = 2) {
  ...
  # Split GL strings by "^" to separate different loci
    ...
    # Combine lists and filter out empty strings
    alleles_list_processed <- c(alleles_list_no_DRB345, alleles_list_DRB345) %>%
    keep(~ str_detect(.x, "[:graph:]"))
  
    # Apply preprocessing to each allele string
    map_chr(alleles_list_processed, preprocess_GL_string, homozygous_count)
    }
...
# The output should be "NA", since both alleles are null, and they are never mismatches.
```
