...
HLA_mismatch_number <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "bidirectional"), homozygous_count = 2){
  valid_directions <- c("HvG", "GvH", "bidirectional")
  direction <- match.arg(direction, valid_directions)
  
  if (direction == "HvG"){
    mismatch_string <- HLA_mismatch_base(GL_string_recip, GL_string_donor, loci, direction, homozygous_count)
    
    # Split the string into loci parts
  loci_parts <- str_split(mismatch_string, ", ")[[1]]
  
  # Use map to iterate over each locus part and count alleles
  allele_counts <- map_chr(loci_parts, function(locus_part) {
    parts <- str_split(locus_part, "=")[[1]]
    locus <- parts[1]
    alleles <- str_split(parts[2], "\\+")[[1]]
    
    # Check for NA and count alleles accordingly
    if (alleles != "NA") {
      count <- 0
    } else {
      count <- sum(!is.na(alleles))
    }
    
    # Return a string with the locus and allele count
    paste(locus, count, sep = "=")
  })
  
  # Collapse the results into a single comma-separated string
...
# Also, will need to update the function if a single locus is supplied.
```
