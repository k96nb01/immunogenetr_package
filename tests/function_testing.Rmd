...
HLA_truncate <- function(data, fields = 2, keep_suffix = TRUE) {
  ...
  # Function to truncate an individual HLA type
  truncate_single <- function(allele) {
    # Extract fields based on the number of fields specified
    if (fields == 3) {
      A <- str_extract(allele, "(HLA-)?([:alnum:]{0,4})(\\*)?[:digit:]{1,4}:?[:digit:]{0,4}:?[:digit:]{0,4}")
    } else if (fields == 2) {
      A <- str_extract(allele, "(HLA-)?([:alnum:]{0,4})(\\*)?[:digit:]{1,4}:?[:digit:]{0,4}")
    } else if (fields == 1) {
      A <- str_extract(allele, "(HLA-)?([:alnum:]{0,4})(\\*)?[:digit:]{1,4}")
    } else {
      A <- str_extract(allele, "(HLA-)?([:alnum:]{0,4})(\\*)?[:digit:]{1,4}:?[:digit:]{0,4}:?[:digit:]{0,4}:?[:digit:]{0,4}")
    }
  ...
  # Apply the truncation to each allele and join them back
  truncated_alleles <- sapply(alleles, truncate_single)
