...
HLA_mismatch_HvG("HLA-A*30:01^HLA-B*15:01", "HLA-A*30:01+HLA-A*02:01^HLA-B*15:01+HLA-B*14:01", c("HLA-A", "HLA-B"))
```
...
HLA_mismatch_HvG_alleles <- function(GL_string_recip, GL_string_donor, loci) {
  ...
  for (locus_index in seq_along(loci)) {
    ...
    # This time we're keeping the full allele nomenclature.
    allele_mismatches_str <-
      if (length(mismatched_alleles) > 0) {
        paste0(locus, ":", paste(mismatched_alleles, collapse = "+"))
      } else {
        paste0(locus, ":NA")
      }
...
HLA_mismatch_HvG_alleles("HLA-A*02:01+HLA-A*30:01^HLA-B*15:01", "HLA-A*02:02+HLA-A*14:01^HLA-B*15:01+HLA-B*14:01", c("A", "B"))
```
...
HLA_mismatch_HvG_number("HLA-A*02:01+HLA-A*68:01^HLA-B*15:02+HLA-B*15:01", "HLA-A*03:01+HLA-A*03:01^HLA-B*15:01+HLA-B*15:01", c("A", "B"), homozygous_count = 2)
```
