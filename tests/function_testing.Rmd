...
```{r}
# Updated HLA_mismatch_HvG function, written with help from PennDnA (Penn's Microsoft copilot service).
...
HLA_mismatch_HvG <- function(GL_string_recip, GL_string_donor, loci) {
  ...
  recip_alleles_list <- strsplit(GL_string_recip, "\\^")[[1]]
  donor_alleles_list <- strsplit(GL_string_donor, "\\^")[[1]]

  # Normalize loci names by removing "HLA-" prefix and any "*" characters
  normalized_loci_recip <- gsub("HLA-", "", loci)
  normalized_loci_donor <- gsub("HLA-", "", loci)
  ...
  missing_loci_names <- setdiff(normalized_loci_recip, intersect(available_loci_recip, available_loci_donor))
  ...
  # Initialize result vector
   mismatch_results <- vector("character", length(loci))
   ...
   for (locus_index in seq_along(loci)) {
     ...
     # Store result as a string "Locus:Mismatch"
     mismatch_results[locus_index] <- paste0(locus, ":", ifelse(mismatch, "TRUE", "FALSE"))
   }
...
HLA_mismatch_HvG("HLA-A*02:01+HLA-A*30:01^HLA-B*15:01", "HLA-A*30:01+HLA-A*68:01^HLA-B*15:01", c("A", "B"))
```

```{r}
# HLA_mismatch_HvG_alleles function 
  ...
  for (locus_index in seq_along(loci)) {
    ...
    allele_mismatches_str <-
      if (length(mismatched_alleles) > 0) {
        paste0(locus, ":", paste(gsub("HLA-.*\\*", "", mismatched_alleles), collapse = "+"))
      } else {
        paste0(locus, ":NA")
      }

    # Store result as a string "Locus:MismatchedAllele(s)"
    mismatch_results[locus_index] <- allele_mismatches_str
  }

  # Collapse all results into a single comma-separated string and return it
  return(paste(mismatch_results, collapse = ","))
}

HLA_mismatch_HvG_alleles("HLA-A*02:01+HLA-A*30:01^HLA-B*15:01", "HLA-A*02:01+HLA-A*30:01^HLA-B*15:01+HLA-B*14:01", c("HLA-A", "HLA-B"))
```

```{r}
# HLA_mismatch_HvG_number function

HLA_mismatch_HvG_number <- function(GL_string_recip, GL_string_donor, loci, homozygous_count = 1) {
  # Split GL strings by "^" to separate different loci
  recip_alleles_list <- strsplit(GL_string_recip, "\\^")[[1]]
  donor_alleles_list <- strsplit(GL_string_donor, "\\^")[[1]]
  ...
  if (length(donor_alleles) == 2 && donor_alleles[1] == donor_alleles[2] && homozygous_count == 1) {
    donor_alleles <- donor_alleles[1]
  }

    # Find mismatched alleles: present in the donor but not in the recipient
	mismatch_count <-
    sum(!(donor_alleles %in% recip_alleles))

	# Create a string representation of mismatch count per locus or '0' if no mismatches are found 
  mismatch_results[locus_index] <-
    paste0(locus, ":", ifelse(mismatch_count > 0, mismatch_count, "0"))
...
HLA_mismatch_HvG_number("A*02:01+A*68:01^B*15:01", "A*03:01+A*03:01^B*15:01+B*14:01", c("A", "B"), homozygous_count = 1)
```
