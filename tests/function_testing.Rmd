...
# 1. The ability to treat the DRB3, DRB4 and DRB5 loci as one locus, so mismatches are calculated between these three loci. I tried for a long time to get this to work with Copilot, but could not.
...
HLA_mismatched_alleles_GvH <- function(GL_string_recip, GL_string_donor, loci, homozygous_count = 2) {
  ...
  process_pair <- function(recip_str, donor_str) {
    ...
    names(recip_alleles_list) <- map_chr(strsplit(recip_alleles_list, "\\*"), 1)
    names(donor_alleles_list) <- map_chr(strsplit(donor_alleles_list, "\\*"), 1)
    
    # Find which supplied loci are missing from recipient and donor typings
    missing_loci_from_recipient <- setdiff(loci, names(recip_alleles_list))
    missing_loci_from_donor <- setdiff(loci, names(donor_alleles_list))
    ...
    mismatch_results <- map(loci, function(locus_name) {
      ...
      recip_alleles_str <- recip_alleles_list[locus_name]
      donor_alleles_str <- donor_alleles_list[locus_name]
