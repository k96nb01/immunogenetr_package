...
# Null alleles (ending in N) should never be output as a mismatch.
# The homozygous count argument is not used.
```
...
```{r}
recip_str <- "HLA-A*03:01^HLA-DRB3*03:01^HLA-DRB5*02:21"
donor_str <- "HLA-A*03:01+HLA-A*74:01^HLA-DRB3*03:01^HLA-DRB5*02:21"
...
# Split GL strings by "^" to separate different loci
    ...
    recip_alleles_list_no_DRB345 <- recip_alleles_list %>% keep(!str_detect(., "HLA-DRB[345]"))
    (recip_alleles_list_processed <- recip_alleles_list_no_DRB345 %>% append(recip_alleles_list_DRB345) %>% keep(~ str_detect(.x, "[:graph:]") ))
    (recip_alleles_list_processed_nulls <- map_chr(recip_alleles_list_processed, preprocess_GL_string, 2))
    ...
    # I think this is working as intended. Will have to incorporate into the main function.
```
