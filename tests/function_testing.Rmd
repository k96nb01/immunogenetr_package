...
```{r}
# Update the base mismatch function.
...
HLA_mismatch_base <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH"), homozygous_count = 2) {
  ...
  # Process the GL string inputs to remove null alleles, and to duplicate alleles if only one remains.
  ...
  # Duplicate single allele if homozygous_count is set to 2
  if (length(non_null_alleles) == 1 && homozygous_count == 2) {
    non_null_alleles <- rep(non_null_alleles, times = homozygous_count)
  }
  
  # Delete the second allele if two of the same alleles were included in the GL string and homozygous_count is set to 1
  if (length(non_null_alleles) == 2 && non_null_alleles[1] == non_null_alleles[2] && homozygous_count == 1) {
    non_null_alleles <- non_null_alleles[1]
  }
  ...
  # Split GL strings by "^" to separate different loci
    ...
    recip_alleles_list_processed <- map_chr(recip_alleles_list_processed, preprocess_GL_string, homozygous_count)
    ...
    donor_alleles_list_processed <- map_chr(donor_alleles_list_processed, preprocess_GL_string, homozygous_count)
    ...
    mismatch_results <- map(loci, function(locus_name) {
      ...
      if (direction == "GvH") {
        ...
        mismatched_allele_1 <- setdiff(unlist(strsplit(recip_alleles_str,"\\+"))[1], unlist(strsplit(donor_alleles_str,"\\+")))
        mismatched_allele_2 <- setdiff(unlist(strsplit(recip_alleles_str,"\\+"))[2], unlist(strsplit(donor_alleles_str,"\\+")))
        ...
        mismatched_allele_1 <- setdiff(unlist(strsplit(donor_alleles_str,"\\+"))[1], unlist(strsplit(recip_alleles_str,"\\+")))
        mismatched_allele_2 <- setdiff(unlist(strsplit(donor_alleles_str,"\\+"))[2], unlist(strsplit(recip_alleles_str,"\\+")))
        mismatched_alleles <- discard(c(mismatched_allele_1, mismatched_allele_2), is.na)
        } else {
...
test_cases %>% mutate(matching_GvH = HLA_mismatch_base(recip_type, donor_type, c("HLA-A", "HLA-B"), direction = "GvH"))
```
...
HLA_mismatch_number <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "bidirectional"), homozygous_count = 2) {
  ...
  if (!is.list(loci) && length(loci) == 1) {
    loci <- list(loci)
  }
  if (direction %in% c("HvG", "GvH")) {
    mismatch_string <- HLA_mismatch_base(GL_string_recip, GL_string_donor, loci, direction, homozygous_count)
    
    # Count mismatches for each locus
    loci_parts <- str_split(mismatch_string, ", ")[[1]]
    allele_counts <- map_chr(loci_parts, ~ {
      parts <- str_split(.x, "=")[[1]]
      locus <- parts[1]
      count <- ifelse(parts[2] == "NA", 0, length(str_split(parts[2], "\\+")[[1]]))
      paste(locus, count, sep = "=")
    })
    
    return(str_c(allele_counts, collapse = ", "))
  } else {
    stop("Direction must be 'HvG' or 'GvH'")
  }
}
...
# Single locus does not appear to be working (this should ouput "2", but instead outputs "HLA-A*03:01+HLA-A*74:01=NA")
HLA_mismatch_number(
  "HLA-A*03:01+HLA-A*74:01^HLA-DRB3*03:01^HLA-DRB5*02:21", 
  "HLA-A*03:02+HLA-A*20:01^HLA-DRB3*03:01",
  "HLA-A",
...
# We should add an output for bidirectional, which should be the max of the GvH and the HvG.
```
