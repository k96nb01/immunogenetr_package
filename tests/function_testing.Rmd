...
```{r}
HLA_mismatched_alleles <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH"), homozygous_count = 2) {
  ...
  process_pair <- function(recip_str, donor_str) {
    ...
    # Process DRB345 alleles to add them to a single locus
    ...
    # Modify the names if they start with DRB3, 4, or 5
    ...
    mismatch_results <- map(loci, function(locus_name) {
      ...
      recip_homozygous <- length(unique(unlist(strsplit(recip_alleles_str,"\\+")))) == 1
      donor_homozygous <- length(unique(unlist(strsplit(donor_alleles_str,"\\+")))) == 1
      
      if (direction == "GvH"){
      # Find mismatched GvH alleles: present in the recipient but not in the donor.
      ...
      if (recip_homozygous && !is.null(mismatched_alleles)) {
      ...
      # Duplicate the mismatch if it is homozygous.
      if (donor_homozygous && !is.null(mismatched_alleles)) {
        mismatched_alleles <- rep(mismatched_alleles, homozygous_count)
      }
...
HLA_columns_to_GLstring <- function(data, HLA_typing_columns, prefix_to_remove = "", suffix_to_remove = "") {
  ...
  step1 <- data %>%
    ...
    # pivoting longer to get each allele on a separate row.
    pivot_longer(cols = all_of(col2mod), names_to = "names", values_to = "allele") %>% 
    # Determine if typing is serrologic by presence of ":" in allele.
    ...
    mutate(locus_from_name = case_when(
      ...
      str_detect(truncated_names, regex("^DQA1[12]?([0-9]*)?$", ignore_case = TRUE)) ~ "HLA-DQA1",  # Molecular DQA1
      str_detect(truncated_names, regex("^DPA1[12]?([0-9]*)?$", ignore_case = TRUE)) ~ "HLA-DPA1",  # Molecular DPA1
      str_detect(truncated_names, regex("^DPB1[12]?([0-9]*)?$", ignore_case = TRUE)) ~ "HLA-DPB1",  # Molecular DPB1
      TRUE ~ "unknown"
    )) %>% 
    # Determine if the DR typing is one of the DRB3/4/5 loci
    ...
    mutate(allele = if_else(
      !molecular & !str_detect(final_locus, "DQA1|DQB|DPA1|DPB1|DQ"),  # Avoid modifying molecular notations
 ...
 HLA_columns_to_GLstring(test5, c(A1Cd:mDPB12cd), suffix_to_remove = "Cd")
 HLA_columns_to_GLstring(test4, c(mA1Cd.recipient:mDPB12cd.recipient), prefix_to_remove = "m", suffix_to_remove = "Cd.recipient") # Class II loci not being detected correctly. Add back the old
```
