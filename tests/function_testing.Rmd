...
```
# Below is a working function. Features that still need to be added:
# 2. If only one locus is passed to the function, then the output should only list the mismatched loci, and not have "HLA-A:" added to the beginning of the output. Outputs like "HLA-A:HLA-A*02:01,HLAB:HLA-B*57:01" should only happen if more than one locus was supplied to the function.
# 3. If there are no mismatches, the function should output "NA" instead of a blank string.


```{r}
# Working HLA_mismatched_alleles_GvH function, with handling of DRB3/4/5 loci.
...
HLA_mismatched_alleles_GvH <- function(GL_string_recip, GL_string_donor, loci, homozygous_count = 2) {
  ...
  if (any(str_detect(GL_string_recip, "[|/]") | str_detect(GL_string_donor, "[|/]"))) {
    stop("HLA_mismatch_HvG does not support ambiguous GL strings containing | or /")
  }
  ...
  process_pair <- function(recip_str, donor_str) {
    ...
    recip_alleles_list_processed <- recip_alleles_list_no_DRB345 %>% append(recip_alleles_list_DRB345) %>% keep(~ str_detect(.x, "HLA"))
    ...
    donor_alleles_list_processed <- donor_alleles_list_no_DRB345 %>% append(donor_alleles_list_DRB345) %>% keep(~ str_detect(.x, "HLA"))
    ...
    # Find which supplied loci are missing from recipient and donor typings
    ...
    mismatch_results_clean <- Filter(Negate(is.null), mismatch_results)
    
    paste(mismatch_results_clean , collapse = ",")
    }
  
  results <- map2_chr(GL_string_recip, GL_string_donor, process_pair)
...
HLA_mismatched_alleles_GvH(
  ...
  c("HLA-DQA1", "HLA-DRB1", "HLA-DRB3/4/5")
  )

(test_cases <- tribble(
  ~case, ~recip_type, ~donor_type,
  1,   "HLA-A*02:01+HLA-A*03:04^HLA-B*15:01^HLA-DRB3*01:01^HLA-DRB4*03:01", "HLA-A*02:01+HLA-A*03:04^HLA-B*15:02^HLA-DRB5*02:02",
  2,   "HLA-A*02:01+HLA-A*30:02^HLA-B*15:01+HLA-B*14:03^HLA-DRB3*03:03", "HLA-A*02:17+HLA-A*32:01^HLA-B*15:01+HLA-B*14:03^HLA-DRB3*03:03",
  3,   "HLA-A*11:01+HLA-A*23:01^HLA-B*15:01+HLA-B*14:03^HLA-DRB4*01:03", "HLA-A*03:01+HLA-A*68:02^HLA-B*15:01+HLA-B*14:03^HLA-DRB5*01:01"
))

test_cases %>% mutate(matching_GvH = HLA_mismatched_alleles_GvH(recip_type, donor_type, c("HLA-A", "HLA-B")))
```
