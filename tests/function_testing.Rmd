...
HLA_mismatch_number <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "bidirectional"), homozygous_count = 2){
  ...
  if (direction == "HvG"){
    # If only a single locus was selected in the arguments, output without starting with the locus name followed by an equal sign.
    if (length(loci) == 1) {
      mismatches <- tibble(HLA_mismatch_base(GL_string_recip, GL_string_donor, loci, direction = "HvG"))  %>% 
      rename(mismatches = 1) %>% 
      separate_wider_delim(mismatches, delim = "+", names = c("one", "two"), too_few = "align_start") %>% 
      rowwise() %>% 
      mutate(mismatches = sum(!is.na(one), !is.na(two))) %>% 
      ungroup() %>% 
      pull(mismatches)
    } else {
      # For cases where multiple loci were called.
      mismatches <- tibble(HLA_mismatch_base(GL_string_recip, GL_string_donor, loci, direction = "HvG")) %>% 
      rename(mismatches = 1) %>% 
      separate_longer_delim(mismatches, delim = ", ") %>% 
      separate_wider_delim(mismatches, delim = "=", names = c("locus", "mismatches"), too_few = "align_start") %>% 
      separate_wider_delim(mismatches, delim = "+", names = c("one", "two"), too_few = "align_start") %>% 
      mutate(across(everything(), ~ na_if(., "NA"))) %>% 
      rowwise() %>% 
      mutate(mismatches = sum(!is.na(one), !is.na(two))) %>% 
      ungroup() %>% 
      mutate(mismatches = str_c(locus, "=", mismatches)) %>% 
      summarize(mismatches= str_flatten(mismatches, collapse = ", ")) %>% 
      pull(mismatches)
    }
  }
...
HLA_mismatch_number(
  ...
  "HLA-A*03:01+HLA-A*30:02^HLA-DRB3*03:01",
