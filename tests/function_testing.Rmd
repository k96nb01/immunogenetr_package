...
HLA_mismatched_alleles <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "Bidirectional"), homozygous_count = 2) {
  ...
  process_pair <- function(recip_str, donor_str) {
    ...
    recip_alleles_list_DRB345 <- recip_alleles_list %>% keep(str_detect(., "HLA-DRB[345]")) %>% paste(collapse = "+")
    recip_alleles_list_no_DRB345 <- recip_alleles_list %>% keep(!str_detect(., "HLA-DRB[345]"))
    recip_alleles_list_processed <- recip_alleles_list_no_DRB345 %>% append(recip_alleles_list_DRB345) %>% keep(~ str_detect(.x, "[:graph:]") )
    
    donor_alleles_list_DRB345 <- donor_alleles_list %>% keep(str_detect(., "HLA-DRB[345]")) %>% paste(collapse = "+")
    donor_alleles_list_no_DRB345 <- donor_alleles_list %>% keep(!str_detect(., "HLA-DRB[345]"))
    donor_alleles_list_processed <- donor_alleles_list_no_DRB345 %>% append(donor_alleles_list_DRB345) %>% keep(~ str_detect(.x, "[:graph:]") )

    # Process DR51/52/53 alleles to add them to a single locus "HLA-DR51/52/53"
    recip_alleles_list_DR51_53 <- recip_alleles_list %>% keep(str_detect(., "HLA-DR5[1-3]")) %>% paste(collapse = "+")
    recip_alleles_list_no_DR51_53 <- recip_alleles_list_processed %>% keep(!str_detect(., "HLA-DR5[1-3]"))
    recip_alleles_list_processed <- recip_alleles_list_no_DR51_53 %>% append(recip_alleles_list_DR51_53) %>% keep(~ str_detect(.x, "[:graph:]") )
    
    donor_alleles_list_DR51_53 <- donor_alleles_list %>% keep(str_detect(., "HLA-DR5[1-3]")) %>% paste(collapse = "+")
    donor_alleles_list_no_DR51_53 <- donor_alleles_list_processed %>% keep(!str_detect(., "HLA-DR5[1-3]"))
    donor_alleles_list_processed <- donor_alleles_list_no_DR51_53 %>% append(donor_alleles_list_DR51_53) %>% keep(~ str_detect(.x, "[:graph:]") )
    
    # Set names for each locus
    names(recip_alleles_list_processed) <- map_chr(
      recip_alleles_list_processed,
      ~ ifelse(str_detect(.x, "\\*"), # Molecular nomenclature
               strsplit(.x, "\\*")[[1]][1],
    ...
    # Modify the names if they start with DR51, 52, 53
    names(recip_alleles_list_processed) <- modify_if(names(recip_alleles_list_processed), ~ str_detect(.x, "HLA-DR5[1-3]"), ~ "HLA-DR51/52/53")
    names(donor_alleles_list_processed) <- modify_if(names(donor_alleles_list_processed), ~ str_detect(.x, "HLA-DR5[1-3]"), ~ "HLA-DR51/52/53")
    ...
    mismatch_results <- map(loci, function(locus_name) {
      ...
      if (is.null(recip_alleles_str) || is.null(donor_alleles_str)) {
        return(NULL)
      }
      
      # Identify mismatched alleles based on direction
      mismatched_alleles <- c()
      
      if (direction == "GvH") {
        # Mismatches present in the recipient but not in the donor
        mismatched_alleles <- setdiff(unlist(strsplit(recip_alleles_str,"\\+")), 
                                      unlist(strsplit(donor_alleles_str,"\\+")))
      } else if (direction == "HvG") {
        # Mismatches present in the donor but not in the recipient
        mismatched_alleles <- setdiff(unlist(strsplit(donor_alleles_str,"\\+")), 
                                      unlist(strsplit(recip_alleles_str,"\\+")))
      } else if (direction == "Bidirectional") {
        # Bidirectional: Mismatches from recipient not in donor OR donor not in recipient
        mismatched_alleles_gvh <- setdiff(unlist(strsplit(recip_alleles_str, "\\+")), 
                                          unlist(strsplit(donor_alleles_str, "\\+")))
        mismatched_alleles_hvg <- setdiff(unlist(strsplit(donor_alleles_str, "\\+")), 
                                          unlist(strsplit(recip_alleles_str, "\\+")))
        mismatched_alleles <- unique(c(mismatched_alleles_gvh, mismatched_alleles_hvg))
      } else {
        stop("Direction must either be 'GvH', 'HvG', or 'Bidirectional'.")
      }
    ...
    # If only a single locus was selected in the arguments, output without starting with the locus name followed by a colon.
    if (length(loci) == 1) {
...
# This is an example of a GL string with mixed serologic and molecular nomenclature, for which the function will need to be updated to handle properly.
# Note that in serologic nomenclature, the DRB3/4/5 loci all start with DR5: DRB3 = DR52, DRB4 = DR53, and DRB5 = DR51.
HLA_mismatched_alleles(
  "HLA-A2+HLA-A68^HLA-DR1+HLA-DR17^HLA-DR52^HLA-DPB1*04:01", 
  "HLA-A3+HLA-A69^HLA-DR4+HLA-DR17^HLA-DR52+HLA-DR53^HLA-DPB1*04:01+HLA-DPB1*04:02",
  # function works when "HLA-DR51/52/53" is removed
  c("HLA-A", "HLA-DR", "HLA-DR51/52/53","HLA-DPB1"),
  direction = "GvH"
  )
...
test_cases %>% mutate(matching_GvH = HLA_mismatched_alleles(recip_type, donor_type, c("HLA-A", "HLA-B"), direction = "HvG"))
```


```{r}
# Play around with some test data in serologic format

(test5) # This is a common format for low-resolution typing. The columns A1Cd:dq2cd are in serologic nomenclature. Note that there are columns for "Bw", which is part of the B locus, but should be treated as a separate locus. Then, the columns mDQA11Cd:mDPB12cd are in molecular nomenclature.

# Test out making a GL string from this low-resolution table.
output <- test5 %>% mutate(GL_string = HLA_columns_to_GLstring(., c(A1Cd:mDPB12cd), suffix_to_remove = "Cd", serologic = TRUE), .after = case)

print(output)
# This does not work as intended. The beginning of the GL string looks good ("HLA-A2^HLA-B7+HLA-B18"), but then the Bw typings are included in the B locus part of the string ("HLA-B7+HLA-B18+HLA-B6+HLA-B6). This part of the string should be "HLA-B7+HLA-B18^HLA-Bw6+HLA-Bw6". Also, using the `serologic = TRUE` argument in the function means that the end of the string improperly records the molecular nomencatlure ("HLA-DQA105:01:01+HLA-DQA104:01:01" should be "HLA-DQA1*05:01:01+HLA-DQA1*04:01:01"). Also, the DR and DQ typing is not correct: "HLA-DR117^HLA-DR28" should be "HLA-DR17^HLA-DR8". I think we need to update the HLA_columns_to_GLstring function to deal with serologic nomenclature properly. The GL string output for the first column should ultimately look like this:

# "HLA-A2^HLA-B7+HLA-B18^HLA-Bw6+HLA-Bw6^HLA-Cw7+HLA-Cw5^HLA-DR17+HLA-DR8^HLA-DR52^HLA-DQ2+HLA-DQ4^HLA-DQA1*05:01:01+HLA-DQA1*04:01:01^HLA-DPA1*01:03:01^HLA-DPB1*02:02:01+HLA-DPB1*03:01:01"
