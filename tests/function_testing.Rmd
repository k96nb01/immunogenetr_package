...
HLA_mismatch_number <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "bidirectional"), homozygous_count = 2) {
  ...
  count_mismatches <- function(mismatch_string) {
    ...
    if (any(is.na(mismatch_string)) || mismatch_string == "NA") {
      return(0L)
    }
  ...
  calculate_mismatch_counts <- function(mismatch_result, loci) {
    ...
    # For multiple loci, process the mismatch result by locus
...
# This works, but doesn't give the correct output:
...
# This works:
test_cases %>% mutate(mismatch = HLA_mismatch_logical(recip_type, donor_type, c("HLA-A", "HLA-B"), direction = "bidirectional", homozygous_count = 2))

# And now this works, too.:
test_cases %>% mutate(mismatch = HLA_mismatch_logical(recip_type, donor_type, c("HLA-A"), direction = "bidirectional", homozygous_count = 2))
```


```{r}
# Rewrite HLA_mismatch_number from scratch
HLA_mismatch_number <- function(GL_string_recip, GL_string_donor, loci, direction = c("HvG", "GvH", "bidirectional"), homozygous_count = 2){
...
# This works properly:
HLA_mismatch_number(
  "HLA-A*02:01+HLA-A*03:04^HLA-B*15:01^HLA-DRB3*01:01", 
  "HLA-A*02:03+HLA-A*32:01^HLA-B*15:01+HLA-B*14:03",
  c("HLA-A"),
  direction = "bidirectional"
)

# This works:
test_cases %>% mutate(mismatch = HLA_mismatch_number(recip_type, donor_type, c("HLA-A", "HLA-B"), direction = "HvG", homozygous_count = 2))

# This works, too:
test_cases %>% mutate(mismatch = HLA_mismatch_number(recip_type, donor_type, c("HLA-A"), direction = "HvG", homozygous_count = 2))
```
...
# This works properly:
HLA_mismatch_logical(
  "HLA-A*02:01+HLA-A*32:01^HLA-B*15:01^HLA-DRB3*01:01", 
  "HLA-A*02:01^HLA-B*15:01+HLA-B*14:03",
  c("HLA-A", "HLA-B"),
  direction = "GvH"
)

# This works:
test_cases %>% mutate(mismatch = HLA_mismatch_logical(recip_type, donor_type, c("HLA-A", "HLA-B"), direction = "HvG", homozygous_count = 2))

# This works, too:
test_cases %>% mutate(mismatch = HLA_mismatch_logical(recip_type, donor_type, c("HLA-A"), direction = "HvG", homozygous_count = 2))
```

```{r}
# Write HLA_mismatched_alleles function.
...
# This works:
test_cases %>% mutate(mismatch = HLA_mismatched_alleles(recip_type, donor_type, c("HLA-A", "HLA-B"), direction = "bidirectional", homozygous_count = 2))

# This works, too:
test_cases %>% mutate(mismatch = HLA_mismatched_alleles(recip_type, donor_type, c("HLA-A"), direction = "bidirectional", homozygous_count = 2))
```

```{r}
# Rewrite the prefix_remove and prefix_add functions
...
# Write the HLA_match_number function
...
HLA_match_number(
  c("HLA-A*02:01+HLA-A*32:01^HLA-B*15:01^HLA-DRB3*01:01", "HLA-A*03:01+HLA-A*34:01^HLA-B*15:02^HLA-DRB3*01:01"), 
  c("HLA-A*02:01+HLA-A*32:01^HLA-B*15:01+HLA-B*14:03", "HLA-A*02:09+HLA-A*36:01^HLA-B*15:01+HLA-B*14:03"),
  c("HLA-A", "HLA-B"),
  "GvH"
)

# This works:
test_cases %>% mutate(matches = HLA_match_number(recip_type, donor_type, c("HLA-A", "HLA-B"), "GvH"))

# This works, too:
test_cases %>% mutate(matches = HLA_match_number(recip_type, donor_type, c("HLA-A"), "GvH"))
```


```{r}
# Write the HLA_match_summary_HCT function.



recip <- c("HLA-A*29:02^HLA-C*06:02+HLA-C*07:01^HLA-B*08:01+HLA-B*13:02^HLA-DRB1*04:01+HLA-DRB1*07:01^HLA-DQB1*02:02+HLA-DQB1*03:02", "HLA-A*29:02+HLA-A*30:02^HLA-C*06:02+HLA-C*07:01^HLA-B*08:01+HLA-B*13:02^HLA-DRB1*04:01+HLA-DRB1*07:01^HLA-DQB1*02:02+HLA-DQB1*03:02")
donor <- c("HLA-A*02:01+HLA-A*29:02^HLA-C*06:01+HLA-C*07:02^HLA-B*08:01+HLA-B*13:03^HLA-DRB1*04:01+HLA-DRB1*07:01^HLA-DQB1*02:02+HLA-DQB1*03:02", "HLA-A*29:02+HLA-A*30:02^HLA-C*06:02+HLA-C*07:01^HLA-B*08:01+HLA-B*13:02^HLA-DRB1*04:01+HLA-DRB1*07:01^HLA-DQB1*02:02+HLA-DQB1*03:02")

HLA_match_summary_HCT(recip, donor, "GvH", "Xof10")
```

```{r}
# Write the HLA_allele_GLstring_regex function
...
GL_string <- "HLA-A2~HLA-C*06:02+HLA-C*07:01|HLA-C*06:03+HLA-C*07:09^HLA-B1+HLA-B*13:02/HLA-B*13:03^HLA-DRB1*04:01+HLA-DRB1*07:01^HLA-DQB1*02:02+HLA-DQB1*03:02?HLA-DPB1*04:01"
Allele <- c("HLA-B1", "HLA-C*07:09")

GLstring_regex(Allele)

str_view(GL_string, GLstring_regex(Allele))

```

```{r}
# Work on how the HLA_mismatch_base function handles the DR51/52/53 loci
...
recip_sero <- "HLA-DR51^HLA-DR52^HLA-DR15+HLA-DR17"
donor_sero <- "HLA-DR51^HLA-DR15+HLA-DR1"

recip_mol <- "HLA-DRB5*01:01^HLA-DRB3*01:01^HLA-DRB1*15:01+HLA-DRB1*03:01"
donor_mol <- "HLA-DRB5*01:01^HLA-DRB1*15:01+HLA-DRB1*01:01"

HLA_match_number(recip_sero, donor_sero, "HLA-DR51/52/53", "GvH")
HLA_match_number(recip_mol, donor_mol, "HLA-DRB3/4/5", "GvH")
```
